...
from dataset import smile_dict

class PreModel(nn.Module):
    def __init__(self,
                 gconv_in_dim: int,
                 gconv_hidden_dim: int,
                 gconv_n_layers: int,
                 feat_dim: int,
                 seq_len=200,seq_dropout=0.1,
                 edge_in_dim=11,edge_mid_dim=128,
                 mask_rate = 0.5,
                 ):
        ...
        self._drop_edge_rate = mask_rate

        self.conv = CNN(len(smile_dict),gconv_hidden_dim,seq_len,seq_dropout,
                        decoder=False,pretraining=False)

        self.gconv = DeeperGCN(gconv_in_dim, gconv_hidden_dim, gconv_n_layers,1,
                               dropout_ratio=0.1, embedding_layer=None,
                               graph_conv=SAGEConvV2,
                               in_edge_channel=edge_in_dim,
                               mid_edge_channel=edge_mid_dim,aggr='softmax')
        
        self.feat_lin = Linear(gconv_hidden_dim,feat_dim)
    ...
    def random_masking(self,x):    
        ...
        len_keep = int(L * (1 - self._mask_rate))
    ...
    def forward(self,x):
        x_graphs, x_seq = x
        x_g_batches = x_graphs.batch
        x_i, _ = self.encoding_mask_noise(x_graphs.clone(),self._mask_rate)
        x_i, _ = self.encoding_edge_noise(x_i, self._mask_rate)

        ris,_ = self.gconv(x_i.x, x_i.edge_index, x_i.edge_attr, x_i.batch)

        ris = global_mean_pool(ris,x_g_batches)

        masked_x = self.random_masking(x_seq).double()
        rjs = self.conv(masked_x)
...
class PreModel_Container(LightningModule):
    def __init__(self,
        ...
        self.gconv_in_dim = gconv_in_dim + 2
        self.gconv_mid_channel = gconv_num_hidden
        self.in_edge_dim = in_edge_channel
        self.mid_edge_channel = mid_edge_channel
        self.mask_rate = mask_rate
        self.drop_edge_rate = drop_edge_rate
        
        self.loss_func = NTXentLoss(0,self.batch_size,0.1,True)

        self.model = PreModel(self.gconv_in_dim,self.gconv_mid_channel,self.num_layers,
                              self.gconv_mid_channel,
                              edge_in_dim=self.in_edge_dim,
                              edge_mid_dim=self.mid_edge_channel,
