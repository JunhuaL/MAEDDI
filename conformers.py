...
class ConformerGenerator(object):
  ...
  def __init__(self,
    ...
    pool_size: int, optional (default 50)
      The initial number of conformers to generate without pruning.
      Higher number of conformers will result in higher chances of getting
      unique conformers.
    """
  ...
  def prune_conformers(self, mol: RDKitMol) -> RDKitMol:
    ...
    for i in sort:
      ...
      if np.all(this_rmsd >= self.rmsd_threshold):
        keep.append(i)
        scores.append(this_rmsd.mean())
      else:
        discard.append(i)

    scored_pairs = list(zip(keep,scores))
    keep = sorted(scored_pairs, reverse=True, key=lambda x: x[1])[:self.max_conformers]
  ...
  def get_conformer_rmsd(self,mol: RDKitMol) -> np.ndarray:
    """
    ...
    for i, ref_conf in enumerate(mol.GetConformers()):
      for j, fit_conf in enumerate(mol.GetConformers()):
        if i >= j:
          continue
        #rmsd[i, j] = rdMolAlign.AlignMol(mol,mol,ref_conf.GetId(),fit_conf.GetId(),maxIters=self.pool_size)
        rmsd[i, j] = AllChem.GetBestRMS(mol, mol, ref_conf.GetId(),
                                       fit_conf.GetId(),maxMatches=self.max_conformers*10)
        rmsd[j, i] = rmsd[i, j]
    return rmsd
